...
function countFallingBricksFor(b) {
    var broken = {};
    ...
    while(breakstack.length) {
        var top = breakstack.pop();

        if (b == top || supportedBy[top].every(sb => broken[sb])) {
            if (!broken[top]) {
                broken[top] = true;
                if (b != top) {
                    count++;
...
for(var b = 0; b < settledBricks.length; b++) {
    total += countFallingBricksFor(b);
}
